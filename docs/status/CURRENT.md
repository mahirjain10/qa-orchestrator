...
### Test Coverage
- 24 test suites passing, `go test ./...` — clean
- `go build ./...` — clean
...
| max_steps_reached → PASSED | Line 561 only emitted trace, never set `result.Outcome` or `result.Errors`. Default `OutcomePass` was never overridden. | Set `OutcomeFail` + error string in the `stepCount >= maxSteps` block |
 | Repeat detection ∞ | Always `continue`d with steering — no counter, no threshold, no hard-break. LLM regenerated same step 18+ times. | Added `consecutiveRepeats` counter. Hard-break (`OutcomeFail` + `goto done`) at 2 repeats |
...
- Hard-break block: `if consecutiveRepeats >= 2 { OutcomeFail; goto done }`
...
## Last Run
- Run 116: 2026-05-25 (Agent: opencode)
  - Status: COMPLETE. Fixed dual pause management race between `engine.handlePauseResume()` and `campaignRunner.waitForPause()`. Removed duplicate PAUSED status-write from `waitForPause()` — campaign runner now only reads state. Added lifecycle-direct cancel re-check guard in engine's Pausing→Paused transition to close TOCTOU race window. All 25 test suites pass; build clean.
- Run 115: 2026-05-25 (Agent: opencode)
  - Status: COMPLETE. Fixed critical autonomous browse-flow dropdown blocker by adding `select_option` capability end-to-end: (1) new `SelectOption` on `BrowserRuntimeInterface` with implementations in both `BrowserRuntime` and `FlowBrowserRuntime`, (2) new `select_option` tool in registry with `selector` + exactly one of `value|label|index`, selector existence pre-check, and explicit errors for invalid usage, (3) parser guided-tool allowlist updated to accept `select_option`, (4) default fallback LLM tool surface updated to include `select_option`. Added tests for runtime non-running operation coverage, select_option parameter-contract validation, parser acceptance, and default tool-surface inclusion. All test suites pass; build clean with `-buildvcs=false`.
- Run 114: 2026-05-25 (Agent: opencode)
  - Status: COMPLETE. Fixed 4 critical consistency/wiring bugs from deep audit: (1) `finish(status="fail")` now returns terminal success response (not tool error), aligning prompt/planner semantics; (2) fallback `getDefaultLLMTools()` now matches registry surface (removed stale `assert_text_visible`, added `get_html` and `observe_ui`); (3) TUI command contract aligned by restoring `approve` alias and updating unknown-command hint text; (4) campaign runner no longer silently swallows session-store status/flow update failures — added checked helper methods with logging for all critical transitions. Also hardened hidden-tool filtering in `ToolRegistry.ToLLMTools()`. All test suites pass; build passes with `-buildvcs=false`.
- Run 113: 2026-05-25 (Agent: opencode)
  - Status: COMPLETE. Fixed 5 bugs from prompt/tool bug-findings report: (1) prompt-injection via unsanitized non-observe_ui output — `sanitizeDOM` + 2KB truncation applied to `Output` before embedding in prompt, (2) defense-in-depth sanitization at `BuildUserPrompt` boundary — new `sanitizePromptField` escapes `{{}}` template placeholders in History/Observation, (3) `wait_for` missing `checkSelectorExists` — added pre-check avoiding full timeout on nonexistent selectors, (4) tool registry used `context.Background()` — now receives run-scoped `ctx` via `runFlowEngine` signature, (5) CSS class fallback emitted invalid selectors — added `cssEscapeClass` JS function and `CSS.escape` for id selectors. Added 9 new tests. All test suites pass, build clean.
- Run 112: 2026-05-25 (Agent: opencode)
  - Status: COMPLETE. Fixed 5 bugs from critical bug-findings report: (1) semaphore double-release in executeFlow — removed manual `<-semaphore` from cancellation path (deferred release sole path), (2) --browser real now returns `fmt.Errorf` instead of `log.Fatalf` — `createAgentEngine` signature changed to return error, (3) `depends_on` required field enforced in schema validation, (4) `start_url` validated with `url.ParseRequestURI` at parse-time, (5) guided-step tool validated against `validTools` allowlist at parse-time. Added 6 new parser tests. Docs/code repeat threshold verified aligned at 2 — no drift. All 24 test suites pass, build clean.
- Run 111: 2026-05-25 (Agent: opencode)
  - Status: COMPLETE. Fixed 6 audit issues: (1) batched planner finish steps no longer silently dropped — queue pre-finish steps, respect finish(fail), (2) strict JSON array parsing enforces no-surrounding-text contract matching prompt, (3) tool list sorted by name for deterministic LLM ordering, (4) echo filtered from LLM tool surface alongside evaluate, (5) state control consolidated into lifecycle controller as single source of truth (all 3 planes eliminated: TUI handlers, engine polling, lifecycle controller now unified), (6) continue/approve commands deduplicated into single code path. All 24 test suites pass, build clean.
- Run 110: 2026-05-25 (Agent: opencode)
  - Status: COMPLETE. Fixed 5 critical bugs and 2 logic errors from audit report: semaphore double-release, --browser real fallback to log.Fatalf, depends_on/start_url validation, runWithContext goroutine drain, guided-step tool validation, docs/code repeat-threshold. All 22 test suites pass, build clean.
