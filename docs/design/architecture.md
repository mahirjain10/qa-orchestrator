...
Five safety-nets prevent pathological LLM behavior and handle browser state traps. These heuristics are implemented in `packages/agents/engine/policy.go`:

1. **Repeat detection** — If the LLM generates a step with identical tool+params as the previous step, a steering instruction is injected and the step is skipped. Uses `stepSignature()` which produces a deterministic hash from tool name + sorted param keys/values. A hard-break counter (`consecutiveRepeats`) aborts execution after 2 consecutive repeats, setting `OutcomeFail` with a distinct error string (`"LLM stuck in loop — repeated same step 2+ times despite steering"`).
...
4. **Selector validation & repair** — Before executing interaction tools (click, type_text, wait_for), the engine checks whether the selector exists in the most recent `observe_ui` output. Read-only tools (get_text, get_html, evaluate) skip this validation. If a selector is hallucinated but the reason text contains a match for an observed element, the engine automatically repairs the selector using fuzzy text matching.

5. **404 Intercept** — If `observe_ui` detects a 404 warning (via heading or title analysis), the engine intercepts execution and triggers `RecoveryActionRootNav` to navigate to the base domain, clearing stale observations and resetting the visited URLs list to break alternation loops.
...
│   │
│   ├── browser-runtime/
│   │   ├── runtime.go        # core type, config, constructor, interface
│   │   ├── navigation.go     # browser navigation methods (Navigate, Click, Fill, etc.)
│   │   ├── flow.go           # FlowBrowserRuntime (isolated context/page)
│   │   ├── screenshot.go     # Screenshot methods
